...
// //               Navigator.pop(ctx);
// //               ref.read(currentUserProvider.notifier).clear();
// //               // Yield to the event loop so Flutter renders one frame
// //               // (AuthGate → SplashScreen) before Firebase signs out.
// //               // Without this, signOut() completes in < 1 frame and
// //               // AuthGate jumps AppShell → LoginPage directly, causing
// //               // the last visible screen to flash for one frame.
...
//           TextButton(
//             onPressed: () async {
//               // Capture refs synchronously — BEFORE Navigator.pop and
//               // before any await. After signOut() completes, Firebase
//               // fires, AuthGate rebuilds, ProfileScreen unmounts, and
//               // accessing `ref` would throw 'widget about to be unmounted'.
//               final logoutNotifier = ref.read(
//                 isLoggingOutProvider.notifier,
//               );
//               final authSvc = ref.read(authServiceProvider);
//               Navigator.pop(ctx);
//               // Flag tells AuthGate to show SplashScreen immediately
//               // on the next frame — before fbUser turns null.
//               logoutNotifier.start();
//               await authSvc.signOut();
//               // Do NOT touch `ref` here — widget is unmounted.
//               // AuthGate auto-resets isLoggingOut via postFrameCallback.
//             },
//             child: const Text(
//               'Log Out',
//               style: TextStyle(
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              // Capture ALL refs before Navigator.pop and before any
              // await. After signOut() Firebase fires, AuthGate rebuilds,
              // ProfileScreen unmounts — touching ref after that throws
              // 'widget about to be unmounted'.
              final logoutNotifier = ref.read(
                isLoggingOutProvider.notifier,
              );
              ...
              Navigator.pop(ctx);
              logoutNotifier
                  .start(); // AuthGate → SplashScreen immediately
              userNotifier
                  .clear(); // prevent ProfileSetup flash on re-login
              await authSvc.signOut();
              // Do NOT touch ref after this — widget is unmounted.
            },
            child: const Text(
              'Log Out',
              style: TextStyle(
