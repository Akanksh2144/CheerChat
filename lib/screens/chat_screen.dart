...
class _ChatScreenState extends State<ChatScreen>
  ...
  void _sendMessage(ChatMessage message) async {
    ...
    if (_isLocked) {
      // TODO: Call process_unlock stored procedure via Node.js API.
      // POST /api/chat/unlock { otherUserId: widget.otherUserId }
      // Deducts 20 coins from wallet_ledger.
      // Only update Firestore AFTER the API confirms success.
      // For now, show a placeholder prompt.
      _showUnlockPrompt();
      return;
    }
  ...
  void _showUnlockPrompt() {
    showDialog<void>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Messaging this host costs 20 coins and unlocks the chat for 7 days.',
        ),
        actions: [
          TextButton(
          ...
          FilledButton(
            ...
            onPressed: () {
              ...
              // TODO: trigger recharge/unlock flow via Node.js API
            },
            child: const Text('Unlock — 20 coins'),
          ),
        ],
      ),
    );
  ...
  Future<void> _pickImage() async {
    ...
    try {
      ...
      // Mark as failed so the user can see and retry
      await messageRef.update({'isUploadFailed': true});
    }
  }
  ...
  void initState() {
    ...
    // Listen to room doc for lock status ONLY — avoids rebuild on every message
    _roomSubscription = FirebaseFirestore.instance
        .collection('conversations')
        .doc(chatRoomId)
        .snapshots()
        .listen((doc) {
          ...
          // Only rebuild if the lock-relevant fields changed
          final bool isUnlocked = data['isUnlocked'] ?? false;
          final Timestamp? expiryTs = data['expiryTime'];
    ...
    // Single stream for read-receipt side effect only
    _messageSubscription = _chatService
        .getMessages(widget.otherUserId)
        .listen((event) {
          if (mounted && event.docChanges.isNotEmpty) {
  ...
  // Fix #4: use View.of(context) instead of deprecated window
  @override
  void didChangeMetrics() {
    final view =
        ...
        actions: [
          Container(
            ...
            child: Material(
              color: Colors.transparent,
              child: InkWell(
                // TODO: pushScreenWithoutNavBar(context, CallInitScreen(otherUserId: widget.otherUserId))
                onTap: () {},
                borderRadius: BorderRadius.circular(200),
                child: const Padding(
